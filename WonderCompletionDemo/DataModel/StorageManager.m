...
/**
 ...
 create the various instance data for Core Data
 check database existence
 check database version, main cause of programmers headache ;-)
 
 for better reuse this class should receive the database name during init and not be hardcoded
 */
...
- (NSManagedObjectModel *)managedObjectModel
{
    ...
    NSURL *modelURL = [[NSBundle mainBundle] URLForResource:@"WonderCompletionDemo" withExtension:@"momd"];
...
- (BOOL)invalidUserVersion:(NSString *)dbPath {
    ...
    if(sqlite3_prepare_v2(dbHandler, "PRAGMA user_version;", -1, &stmt_version, NULL) == SQLITE_OK) {
        ...
        NSLog(@"%s: DatabaseVersion is: %d expected: %d", __FUNCTION__, databaseVersion,cCurrentDatabaseVersion);
    } else {
...
- (NSPersistentStoreCoordinator *)persistentStoreCoordinator
{
    ...
    NSString *storePath = [[self applicationDocumentsDirectoryAsString]
                           stringByAppendingPathComponent: @"WonderCompletionDemo.sqlite"];
    ...
    if (oldVersion | (![fileManager fileExistsAtPath:storePath])) {
        NSString *defaultStorePath = [[NSBundle mainBundle]
                                      pathForResource:@"WonderCompletionDemo" ofType:@"sqlite"];
        if (defaultStorePath) {
